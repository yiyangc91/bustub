...
bool BufferPoolManager::UnpinPageImpl(page_id_t page_id, bool is_dirty) {
	...
	--pages_[frameId].pin_count_;
	LOG_DEBUG("UnpinPageImpl(%d, %d): Decrementing pin count to %d",
			page_id, is_dirty, pages_[frameId].pin_count_);
	if (pages_[frameId].pin_count_ == 0) {
...
Page *BufferPoolManager::NewPageImpl(page_id_t *page_id) {
	...
	LOG_DEBUG("NewPageImpl(%p): Allocated a page %d at frame %d",
			page_id, *page_id, frame);
